...
\begin{subequations}
    \begin{align}
        ...
        =&\frac{1}{2}\ii\int_C\left[-\dd[]{z}\left(\expval{T^{\bar{z}\bar{z}}\epsilon_{\bar{z}}X}+\eqnmarkbox[blue]{node1}{\expval{T^{\bar{z}z}\epsilon_{z}X}}\right)+\dd[]{\bar{z}}\left(\expval{T^{zz}\epsilon_z X}+\eqnmarkbox[blue]{node2}{\expval{T^{z\bar{z}}\epsilon_{\bar{z}} X}}\right)\right]\label{eq:2DCWI:conformal_ward_gauss_1}\\
                                                      =&\frac{1}{2}\ii\int_C\left(-\dd[]{z}\expval{T^{\bar{z}\bar{z}}\epsilon_{\bar{z}}X}+\dd[]{\bar{z}}\expval{T^{zz}\epsilon_z X}\right)\label{eq:2DCWI:conformal_ward_gauss_2}
    \end{align}
...
\begin{boxmath}{Conformal Ward Identity}
    \begin{align}
        \delta_{\epsilon,\bar{\epsilon}}\expval{X}=-\frac{1}{2\pi\ii}\oint_C\dd[]{z}\epsilon(z)\expval{T(z)X}+\frac{1}{2\pi\ii}\oint_C\dd[]{\bar{z}}\bar{\epsilon}(\bar{z})\expval{\bar{T}(\bar{z})X}.\label{eq:2dcft:conformal_ward_final}
    \end{align}
\end{boxmath}
...
\begin{subequations}
    \begin{align}
        ...
        \expval{\partial_{\bar{z}}\phi(z,\bar{z})\partial_{\bar{w}}\phi(w,\bar{w})}=&-\frac{1}{4\pi g}\frac{1}{(\bar{z}-\bar{w})^2}.
    \end{align}
\end{subequations}
In the following we shall concentrate on the holomorphic part $\partial\phi\equiv\partial_z\phi$.
It is clear from \cref{eq:FFOPE:FB:2_p_after_derivative_1} we have the OPE 
\begin{align}
    \partial\phi(z)\partial\phi(w)\sim-\frac{1}{4\pi g}\frac{1}{(z-w)^2}.
\end{align}
\begin{remark}
...
\begin{align}
    T(z)=-2\pi g:\partial\phi\partial\phi:=-2\pi g\lim_{w\to z}\left(\partial\phi(z)\partial\phi(w)-\expval{\partial\phi(z)\partial\phi(w)}\right).
\end{align}
The OPE of $T(z)$ can be calculated using Wick's theorem\sidenote{Don't be too serious on the Wick's theorem we are using here.}:
\begin{subequations}
    \begin{align}
        T(z)\partial\phi(w)&=-2\pi g\lim_{z_1\to z}\left(\partial\phi(z)\partial\phi(z_1)-\expval{\partial\phi(z)\partial\phi(z_1)}\right)\partial\phi(w)\\
                    &\sim -2\pi g\lim_{z_1\to z}\left[\left(\expval{\partial\phi(z)\partial\phi(w)}\partial\phi(z_1)+\expval{\partial\phi(z)\partial\phi(z_1)}\partial\phi(w)+\right.\right.\notag\\
                    &\quad\quad\quad\quad\quad\quad\left.\left.\expval{\partial\phi(z_1)\partial\phi(w)}\partial\phi(z)\right)-\expval{\partial\phi(z)\partial\phi(z_1)}\partial\phi(w)\right]\\
                    &\sim -2\pi g\lim_{z_1\to z}\left(\expval{\partial\phi(z)\partial\phi(w)}\partial\phi(z_1)+\expval{\partial\phi(z_1)\partial\phi(w)}\partial\phi(z)\right)\\
                    &\sim -4\pi g\expval{\partial\phi(z)\partial\phi(w)}\partial\phi(z)\\
                    &\sim \frac{\partial\phi(z)}{(z-w)^2}
    \end{align} 
\end{subequations} 
We used 
Then by expanding $\partial\phi(z)$ around $w$, we have the OPE 
\begin{align}
    T(z)\partial\phi(w)\sim\frac{\partial\phi(w)}{(z-w)^2}+\frac{\partial^2_w\phi(w)}{(z-w)}.
\end{align}

Using the same method, we also have the OPE of the energy-momentum tensor with itself 
\begin{align}
    T(z)T(w)=&4\pi^2 g^2:\partial\phi(z)\partial\phi(z)::\partial\phi(w)\partial\phi(w):\notag\\
...
Using the conformal Ward identity \cref{eq:2dcft:conformal_ward_final} ,we have 
\begin{align}
    \delta_\epsilon T(w)&=-\frac{1}{2\pi\ii}\oint_C\dd[]{z}\epsilon(z)T(z)T(w)\notag\\
                        &=-\frac{1}{12}c\partial^3_w\epsilon(w)-2T(w)\partial_w\epsilon(w)-\epsilon(w)\partial_w T(w).\label{eq:2dcft:central_charge:EMtensor_transform}
\end{align}
The finite form of \cref{eq:2dcft:central_charge:EMtensor_transform} is
\begin{align}
    T'(w)=\left(\dv{w}{z}\right)^{-2}\left[T(z)-\frac{c}{12}\left\{w;z\right\}\right]
\end{align}
