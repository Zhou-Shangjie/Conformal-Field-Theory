...
\begin{intu}
    In the ordinary QFT books, fields with spin $s>0$ are often appear as multiplets.
    But the field with higher spin $\phi$ we see here does not appear as a multiplet.
    We can think of the field in 2D CFT as a complex scalar field defined on an Euclidean background.
    That the Euclidean rotation group is different from the Lorentz group can be seen as a na\"ive reason why the field $\phi$ with higher spin does not appear as a multiplet.
...
\begin{subequations}
    \begin{align}
        \pdv{x^\mu}\expval{\tensor{T}{^\mu_\nu}(x)X}&=-\sum_{i=1}^n\delta(x-x_i)\pdv{x^\nu_i}\expval{X}\\
            \eqnmarkbox[blue]{node1}{\varepsilon_{\mu\nu}}\expval{T^{\mu\nu}(x)X}&=-\ii\sum_{i=1}^n s_i\delta(x-x_i)\expval{X}\\
            \expval{\tensor{T}{^\mu_\mu}(x)X}&=-\sum_{i=1}^n\delta(x-x_i)\Delta_i\expval{X}
    \end{align}
...
\end{align}
Using Ward identities \cref{eq:GCI:conformal_ward_final_1,eq:GCI:conformal_ward_final_2,eq:GCI:conformal_ward_final_3}, \cref{eq:2DCWI:before_ward} becomes 
\begin{align}
