...
\begin{align}
    \expval{\tensor{T}{^\mu_\mu}X}=-\sum_i\delta(x-x_i)\Delta_i\expval{X}
\end{align}
...
The antisymmetric tensor can be written in holomorphic form as\sidenote{For example, $\epsilon_{z\bar{z}}$ can be calculated as \begin{align*}\epsilon_{z\bar{z}}=&\epsilon_{\mu\nu}\dv{z^\mu}{z}\dv{z^\nu}{\bar{z}}\\=&\dv{z^0}{z}\dv{z^1}{\bar{z}}-\dv{z^1}{z}\dv{z^0}{z^\beta}.\end{align*}}
\begin{align}
    \epsilon_{\mu\nu}=\mqty(0&\frac{\ii}{2}\\-\frac{\ii}{2}&0)\quad\epsilon^{\mu\nu}=\mqty(0&-2\ii\\2\ii&0).
\end{align} 
...
\begin{align}
    z\to w(z).\label{eq:2dcft:2d_coformal_transformation_arbitrary}
\end{align}
...
\begin{intu}
    The conformal transformation \cref{eq:2dcft:2d_coformal_transformation_arbitrary} is not all well-defined everywhere and invertible.
...
\begin{align}
    h\equiv\frac{\Delta+s}{2}\quad\bar{h}\equiv\frac{\Delta-s}{2}.
\end{align}
...
\begin{claim}
    Dirac delta function $\delta(x)$ can be written as
    \begin{align}
...
\begin{proof}
    ...
    It is more convenient to use a counterclockwise differential $\dd[]{s^\rho}$ which is parallel to the contour $\partial M$: $\dd[]{\xi_\mu}=\epsilon_{\mu\rho}\dd[]{s^\rho}$.
    Now \cref{eq:2d_ward:gauss} becomes
    \begin{align}
        \int_M\dd[2]{x}\partial_\mu F^\mu=&\int_{\partial M}\dd[]{\xi_\mu} F^\mu\notag\\
                                            =&\int_{\partial M}\epsilon_{\mu\rho}\dd[]{s^\rho}F^\mu\notag\\
                                            =&\int_{\partial M}\left(\epsilon_{\bar{z}z}\dd[]{z}F^{\bar{z}}+\epsilon_{z\bar{z}}\dd[]{\bar{z}}F^{z}\right)\notag\\
                                            =&\frac{\ii}{2}\int_{\partial M}\left(-\dd[]{z}F^{\bar{z}}+\dd[]{\bar{z}}F^{z}\right)
    \end{align} 

    Then we can directly check \cref{eq:2d_ward:dirac_another} for a function $f(z)$ which is analytic within $M$:
    \begin{subequations}
        \begin{align}
            \int_M \dd[2]{x}\delta(x)f(z)=&\frac{1}{\pi}\int_M\dd[2]{x}f(z)\partial_{\bar{z}}\frac{1}{z}\\
                                        =&\frac{1}{\pi}\int_M\dd[2]{x}\partial_{\bar{z}}\left(\frac{f(z)}{z}\right)\\
                                        =&\frac{1}{2\pi \ii}\int_{\partial M}\dd[]{z}\frac{f(z)}{z}\notag\\
                                        =&f(0).
        \end{align}
    \end{subequations}
\end{proof}
