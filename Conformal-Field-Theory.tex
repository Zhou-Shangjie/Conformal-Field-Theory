...
\begin{boxmath}{3-point function}
    \begin{align}
        \expval{\phi_1(x_1)\phi_2(x_2)\phi_3(x_3)}=\frac{C_{123}}{x_{12}^{\Delta_1+\Delta_2-\Delta_3}x_{23}^{\Delta_2+\Delta_3-\Delta_1}x_{13}^{\Delta_3+\Delta_1-\Delta_2}}.
    \end{align}
\end{boxmath}
...
\paragraph{Rotation}
\begin{align}
    \expval{\left(T^{\rho\nu}-T^{\nu\rho}X\right)}=-\ii\sum_i\delta(x-x_i)S^{\nu\rho}_i\expval{X}
\end{align}
...
\end{align}
The condition \cref{eq:ct:conformal_transformation} that defines a conformal transformation is now (see details in \cref{app:derivation:1})
...
\begin{subequations}\label{eq:2dcft:complex_coordinates}
    \begin{align}
        ...
        \partial_{\bar{z}}\equiv & \frac{1}{2}(\partial+\ii\partial_1)
    \end{align}
\end{subequations}
and also 
\begin{subequations}
...
\begin{remark}
    ...
    However, we should keep in mind that the physical space is the 2-dimensional submanifold defined by $\bar{z}=z^\ast$.
\end{remark}
...
\begin{align}
    z\to w(z).\label{eq:2dcft:2d_coformal_transformation_arbitary}
\end{align}
...
\begin{intu}
    The conformal transformation \cref{eq:2dcft:2d_coformal_transformation_arbitary} is not all well-defined everywhere and invertible.
...
\subsection{Derivation of Some Formulas}
\subsubsection{Derivation of \texorpdfstring{\cref{eq:2d_cft:2d_conformal_transformation_1}}{1}\label{app:derivation:1}}
\clearpage
