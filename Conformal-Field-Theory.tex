...
\begin{align}\label{eq:operator_formalism:operator_algebra_ansatz}
    \phi_q(z,\bar{z})\phi_w(0,0)=\sum_p \sum_{\{k,\bar{k}\}}C^{p\{k,\bar{k}\}}_{12}z^{h_p-h_1-h_2+K}\bar{z}^{\bar{h}_p-\bar{h}_1-\bar{h}_2+\bar{K}}\phi_p^{\{k,\bar{k}\}}(0,0)
\end{align}
...
\end{intu}
\begin{example}[Computation of $\beta^{p\{0\}}_{ij}$ and $\bar{\beta}^{p\{0\}}_{ij}$]
