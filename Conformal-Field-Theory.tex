...
\begin{align}
    G^{21}_{34}(x,\bar{x}) & \equiv\lim_{z,\bar{z}\to\infty}z^{2h_1} \bar{z}^{2\bar{h}_1}\expval{\phi_1(z,\bar{z})\phi_2(1,1)\phi_3(x,\bar{x})\phi_4(0,0)} \\
...
The crossing symmetry is a kind of symmetry that the order of operators in the correlator does not affect the value of the correlator\sidenote{We are considering the bosonic theories. In fermionic theories, signs of the correlator may change.}, for example 
\begin{align}
    \expval{\phi_1(x_1)\phi_2(x_2)\phi_3(x_3)\phi_4(x_4)}=\expval{\phi_2(x_2)\phi_1(x_1)\phi_1(x_3)\phi_4(x_4)}.
\end{align}
\begin{intu}
    A CFT in operator formalism should have crossing symmetry to be consistent with the path integral formalism and this condition will give us some non-trivial consistency relation which can be used to fix the correlators.
\end{intu}
\begin{subequations}
    \begin{align}
        G^{21}_{34}(x,\bar{x})&=G^{41}_{32}(1-x,1-\bar{x})\\
